...
function handleGroupSelected(group){
	...
	formData.value.alias = group.alias
	formData.value.description = group.description
	formData.value.tags = group.tags?group.tags:[]
}
...
<template>
	<ElForm ref="formRef" :model="formData" label-position="top" :rules="rules">
		<ElFormItem label="所选用户组" prop="userGroupId">
			<UserGroupSelector v-model="formData.userGroupId" @change="handleGroupSelected" />
		</ElFormItem>
