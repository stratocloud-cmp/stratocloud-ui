...
function confirm(){
	formRef.value.validate(valid=>{
		...
		confirmOrder(formData.value).then(resp=>{
...
function showButton(){
	const allowedStatuses = ['AWAIT_CONFIRM']
...
<template>
	<template v-if="showButton()">
		...
		<teleport to="body">
			<StratoDrawer v-model="drawerFlag" title="确认工单" :loading="drawerLoading" @on-confirm="confirm">
				<ElFormItem v-if="isAnyOrderItemChanged">
					<ElText type="warning">
						检测到当前页面存在工单项修改后未保存，如需保存，请前往工单详情页点击对应的[保存工单项]后再次点击确认
					</ElText>
				</ElFormItem>
